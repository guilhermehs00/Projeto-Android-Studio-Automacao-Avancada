...
public class Medir_Trajetoria_3 extends Thread {
    ...
    public Medir_Trajetoria_3(AtividadePrincipal atividadePrincipal, TextView num_medicao, TextView notification, TextView notification2, TextView labelsizeFila, TextView addRegionButton, TextView GravarBdButtom,
                              Handler uiHandler, Region F1, Region F2, Region F3, Region F4, Region F5, Region F6, double d3[][]) {
        ...
        GravarBdButtom.setOnClickListener(v -> {
            ...
            uiHandler.post(() -> {
                notification.setText(format("Todos deslocamentos\n" +
                                ...
                                "%.4f %.4f %.4f %.4f %.4f\n" +
                                "%.4f %.4f %.4f %.4f %.4f\n",
                        d3[0][0], d3[0][1], d3[0][2], d3[0][3], d3[0][4],
                        d3[1][0], d3[1][1], d3[1][2], d3[1][3], d3[1][4],
                        d3[2][0], d3[2][1], d3[2][2], d3[2][3], d3[2][4],
                        d3[3][0], d3[3][1], d3[3][2], d3[3][3], d3[3][4],
                        d3[4][0], d3[4][1], d3[4][2], d3[4][3], d3[4][4],
                        d3[5][0], d3[5][1], d3[5][2], d3[5][3],d3[5][4],
                        d3[6][0], d3[6][1], d3[6][2], d3[6][3],d3[6][4],
                        d3[7][0], d3[7][1], d3[7][2], d3[7][3],d3[7][4],
                        d3[8][0], d3[8][1], d3[8][2], d3[8][3],d3[8][4],
                        d3[9][0], d3[9][1], d3[9][2], d3[9][3],d3[9][4]
                ));
                notification.setVisibility(View.VISIBLE);
        ...
        while (rodando) {
            ...
            if (Medicao < 10) {
                ...
                if (F2 != null && F2.distance(latitudeAtual, longitudeAtual, F2.getLatitude(), F2.getLongitude())) {
                    if(d3[Medicao][0] == 0.00){
                        if (Medicao == 0){
                            tempo_inicio2 = System.nanoTime();
                            d3[Medicao][0] = (tempo_inicio2-tempo_inicio1) / 1_000_000_000.0;
                            Mostrar_medida(d3[Medicao][0], 0);
                        }else{
                            if (cronometroRodando){
                                tempo_inicio2 = System.nanoTime();
                                d3[Medicao][0] = (tempo_inicio2-tempo_inicio1) / 1_000_000_000.0;
                ...
                if (F3 != null && F3.distance(latitudeAtual, longitudeAtual, F3.getLatitude(), F3.getLongitude())) {
                    if((d3[Medicao][1] == 0.00)&&(d3[Medicao][0] != 0.00)){
                        tempo_inicio3 = System.nanoTime();
                        d3[Medicao][1] = (tempo_inicio3 - tempo_inicio2) / 1_000_000_000.0;
                        Mostrar_medida(d3[Medicao][1], 1);

                    }
                }
                if (F4 != null && F4.distance(latitudeAtual, longitudeAtual, F4.getLatitude(), F4.getLongitude())) {
                    if((d3[Medicao][2] == 0.00)&&(d3[Medicao][1] != 0.00)){
                        tempo_inicio4 = System.nanoTime();
                        d3[Medicao][2] = (tempo_inicio4 - tempo_inicio3) / 1_000_000_000.0;
                        Mostrar_medida(d3[Medicao][2], 2);
                    }
                }
                if (F5 != null && F5.distance(latitudeAtual, longitudeAtual, F5.getLatitude(), F5.getLongitude())) {
                    if((d3[Medicao][3] == 0.00)&&(d3[Medicao][2] != 0.00)){
                        tempo_inicio5 = System.nanoTime();
                        d3[Medicao][3] = (tempo_inicio5 - tempo_inicio4) / 1_000_000_000.0;
                        Mostrar_medida(d3[Medicao][3], 3);
                    }

                }
                if (F6 != null && F6.distance(latitudeAtual, longitudeAtual, F6.getLatitude(), F6.getLongitude())) {
                    if((d3[Medicao][4] == 0.00)&&(d3[Medicao][3] != 0.00)){
                        tempo_inicio6 = System.nanoTime();
                        d3[Medicao][4] = (tempo_inicio6 - tempo_inicio5) / 1_000_000_000.0;
                        Mostrar_medida(d3[Medicao][4], 4);
                        Mostrar_medida(d3, Medicao);
                        Medicao++;
