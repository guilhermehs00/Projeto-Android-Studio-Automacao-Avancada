...
public class Medir_Trajetoria_2 extends Thread {
    ...
    public Medir_Trajetoria_2(AtividadePrincipal atividadePrincipal, TextView num_medicao, TextView notification, TextView notification2, TextView labelsizeFila, TextView addRegionButton, TextView GravarBdButtom,
                   Handler uiHandler, Region F1, Region F2, Region F3, Region F4, Region F5, Region F6, double d2[][]) {
        ...
        while (rodando) {
            ...
            if (Medicao < 10) {
                ...
                if (F2 != null && F2.distance(latitudeAtual, longitudeAtual, F2.getLatitude(), F2.getLongitude())) {
                    if(d2[Medicao][0] == 0.00){
                        if (Medicao == 0){
                            tempo_inicio2 = System.nanoTime();
                            d2[Medicao][0] = (tempo_inicio2-tempo_inicio1) / 1_000_000_000.0;
                            Mostrar_medida(d2[Medicao][0], 0);
                        }else{
                            if (cronometroRodando){
                                tempo_inicio2 = System.nanoTime();
                                d2[Medicao][0] = (tempo_inicio2-tempo_inicio1) / 1_000_000_000.0;
                ...
                if (F3 != null && F3.distance(latitudeAtual, longitudeAtual, F3.getLatitude(), F3.getLongitude())) {
                    if((d2[Medicao][1] == 0.00)&&(d2[Medicao][0] != 0.00)){
                        tempo_inicio3 = System.nanoTime();
                        d2[Medicao][1] = (tempo_inicio3 - tempo_inicio2) / 1_000_000_000.0;
                        Mostrar_medida(d2[Medicao][1], 1);

                    }
                }
                if (F4 != null && F4.distance(latitudeAtual, longitudeAtual, F4.getLatitude(), F4.getLongitude())) {
                    if((d2[Medicao][2] == 0.00)&&(d2[Medicao][1] != 0.00)){
                        tempo_inicio4 = System.nanoTime();
                        d2[Medicao][2] = (tempo_inicio4 - tempo_inicio3) / 1_000_000_000.0;
                        Mostrar_medida(d2[Medicao][2], 2);
                    }
                }
                if (F5 != null && F5.distance(latitudeAtual, longitudeAtual, F5.getLatitude(), F5.getLongitude())) {
                    if((d2[Medicao][3] == 0.00)&&(d2[Medicao][2] != 0.00)){
                        tempo_inicio5 = System.nanoTime();
                        d2[Medicao][3] = (tempo_inicio5 - tempo_inicio4) / 1_000_000_000.0;
                        Mostrar_medida(d2[Medicao][3], 3);
                    }

                }
                if (F6 != null && F6.distance(latitudeAtual, longitudeAtual, F6.getLatitude(), F6.getLongitude())) {
                    if((d2[Medicao][4] == 0.00)&&(d2[Medicao][3] != 0.00)){
                        tempo_inicio6 = System.nanoTime();
                        d2[Medicao][4] = (tempo_inicio6 - tempo_inicio5) / 1_000_000_000.0;
                        Mostrar_medida(d2[Medicao][4], 4);
                        Mostrar_medida(d2, Medicao);
                        Medicao++;
